...
%% Load data
load('UID_feature_mat.mat');
feature_vector_num = 42;

feature_mat_scaled = zeros(size(UID_feature_mat));
for i = 1:size(feature_mat_scaled,2)
    feature_mat_scaled(:,i) = mapminmax(UID_feature_mat(:,i)')';
end
save feature_mat_scaled feature_mat_scaled

UID_feature_mat = feature_mat_scaled;

load('UID_MOS.mat');  %加载主观分数
score_matrix =UID_MOS;
